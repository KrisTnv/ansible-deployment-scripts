...
function string.starts(String,Start)
 ...
 function slurm_job_submit ( job_desc, part_list, submit_uid )
     ...
     elseif shared == 0 then
         slurm.log_info("testcluster_job_submit: Exclusive Job")
     end
 
 
     if min_mem_per_node == nil then
         slurm.log_info("testcluster_job_submit: min_mem_per_node was nil")
     end
 
     if min_mem_per_cpu == nil then
         slurm.log_info("testcluster_job_submit: min_mem_per_cpu was nil")
     end
 
     if min_nodes == nil then
         slurm.log_info("testcluster_job_submit: min_nodes was nil")
     elseif min_nodes == 4294967294 then
         slurm.log_info("testcluster_job_submit: min_nodes was 4294967294")
     elseif min_nodes == 1 then
         slurm.log_info("testcluster_job_submit: min_nodes was 1")
     else
         user_spec_nodes = 1
         slurm.log_info("testcluster_job_submit: user specified min_nodes was %s", min_nodes)
     end
 
     if ntasks_per_node == nil then
         slurm.log_info("testcluster_job_submit: ntasks_per_node was nil")
     else
     ...
     if num_tasks == nil then
         slurm.log_info("testcluster_job_submit: num_tasks was nil")
     elseif num_tasks > 42949672 then
         slurm.log_info("testcluster_job_submit: num_tasks was not provided")
     else
         user_spec_tasks = 1
     end
     ...
     if default_account ~= nil then
         slurm.log_info("testcluster_job_submit: default account is %s", default_account)
     else
         slurm.log_info("testcluster_job_submit: no default account")
     end
 
     if user_account ~= nil then
         slurm.log_info("testcluster_job_submit: account provided %s", user_account)
     else
         slurm.log_info("testcluster_job_submit: no account provided")
     end
 
     if features ~= nil then
         slurm.log_info("testcluster_job_submit: Features supplied is %s", features)
     else
         slurm.log_info("testcluster_job_submit: no features requested")
     end
 
     -- If ask for specific nodes or to exclude nodes, then reject
     if req_nodes ~= nil then
         slurm.log_info("testcluster_job_submit: Specific nodes requested. Reject job")
         slurm.log_user("Job rejected: Please do not request specific nodes.")
         -- return slurm.ERROR
     elseif exc_nodes ~= nil then
         slurm.log_info("testcluster_job_submit: Specific nodes excluded. Reject job")
     ...
     if string.find(qos, "maintenance") then
         slurm.log_info("testcluster_job_submit: maintenance qos specified %s",qos)
     else
             slurm.log_info("testcluster_job_submit: qos specified %s",qos)
         if reservation_name ~= nil then
             if string.find(qos, "reservation") then
                     slurm.log_info("testcluster_job_submit: reservation qos requested with reservation. Continue")
                 else
                     slurm.log_info("testcluster_job_submit: non-reservation qos requested with reservation. Reject job")
         ...
         slurm.log_info("testcluster_job_submit: nodes requested is %s so switches is calculated to be %s",min_nodes,calc_switches)
         ...
         slurm.log_info("testcluster_job_submit: nodes requested is %s so switches is calculated to be %s",min_nodes,calc_switches)
                 ...
                 slurm.log_info("testcluster_job_submit: min_nodes %s is not a power of 2. Reject job.", min_nodes)
                 slurm.log_user("Job rejected: Please request a node count that is a power of 2.")
                 return slurm.ERROR
         end
         if min_nodes == 16 then
         slurm.log_info("testcluster_job_submit: settings grid16 features")
             job_desc.features = "[grid16-01|grid16-02|grid16-03|grid16-04|grid16-05|grid16-06|grid16-07]"
         end
         if min_nodes == 32 then
                 slurm.log_info("testcluster_job_submit: settings grid32 features")
                 job_desc.features = "[grid32-01|grid32-02|grid32-03]"
             end
         if min_nodes == 64 then
                 slurm.log_info("testcluster_job_submit: settings grid64 features")
                 job_desc.features = "[grid64-01|grid64-02]"
             end
     end
     else
     slurm.log_info("testcluster_job_submit: No qos specified. Reject job")
     ...
     slurm.log_info("testcluster_job_submit: Job submission evaluation END for %s", username)
     return slurm.SUCCESS
 end
 
 function slurm_job_modify ( job_desc, job_rec, part_list, modify_uid )
     slurm.log_info("testcluster_job_submit: slurm_job_modify")
